  ...
  const handleSubmit = async (e) => {
    ...
    if (pickupCoords && deliveryCoords) {
      ...
      try {
        ...
        // Create and send notification data to the backend
        const newNotification = {
          userId: formData.contact, // Replace with the relevant user identifier
          message: 'Your rider is successfully booked',
        };

        const notificationResponse = await axios.post('http://localhost:8000/api/notifications', newNotification);
        console.log('Notification stored:', notificationResponse.data);
